//! Rust-side smoke test for the BoltFFI surface. The methods exercised here
//! are the exact ones bolt-ffi exports to Swift/Kotlin/Java/C#/WASM, so a
//! green run gives confidence that the foreign-language bindings will
//! exercise a working pipeline on top of the shared facade.

use jmdict_fast_bolt::{Dict, MatchMode, MatchType, QueryOptions, Xref};
...
#[test]
fn lookup_partial_through_bolt() {
    let dict = load();
...
#[test]
fn lookup_with_options_through_bolt() {
    let dict = load();
...
#[test]
fn iter_entries_paginates_through_bolt() {
    let dict = load();
...
fn dict_is_send_and_sync() {
    // BoltFFI asserts Send + Sync on exported classes; lock it in via the
    // type system so a future change to Dict can't silently regress.
